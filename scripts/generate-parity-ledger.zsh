...
jq --slurpfile keys "$keybindings_path" '
  ...
  def menu_parity:
    ...
    | if $item.referenceKind != "action" then
      ...
      elif ($item.id | test("^(system\\.hibernate|trigger\\.capture\\.screenrecord\\.webcam|trigger\\.hardware\\.(hybrid-gpu|touchscreen)|trigger\\.toggle\\.(window-gaps|one-window-ratio|crash-capture)|style\\.bar\\.(position\\.(bottom|left|right)|transparency)|style\\.unlock|update\\.process\\.hyprsunset|update\\.channel\\.(rc|dev))")) then
