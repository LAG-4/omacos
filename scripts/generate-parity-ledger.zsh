...
jq --slurpfile keys "$keybindings_path" '
  ...
  def cli_parity:
    ...
    | if in_list($item.id; ["drive", "finalize", "hibernation", "hyprland", "plymouth", "sudo", "windows"]) then
        ...
        parity("cli-group"; $item.id; $item.description; "limited"; "close-substitute"; "omacos channel"; "The interface is present, but the current open-source release supports only one tested main channel."; "bin/omarchy")
  ...
  def menu_parity:
    ...
    | if $item.referenceKind != "action" then
      ...
      elif ($item.id | test("^(system\\.hibernate|trigger\\.capture\\.screenrecord\\.webcam|trigger\\.hardware\\.(hybrid-gpu|touchscreen)|trigger\\.toggle\\.(window-gaps|one-window-ratio|crash-capture)|style\\.bar\\.(position\\.(bottom|left|right)|transparency)|style\\.unlock|update\\.process\\.hyprsunset|update\\.channel($|\\.))")) then
      ...
      elif ($item.id | test("^(apps$|about$|system\\.|learn\\.|trigger\\.(emoji|reminder|capture\\.(screenshot|text|color|qr|screenrecord\\.(no-audio|desktop-audio|microphone|stop))|transcode|share($|\\.)|toggle\\.(idle-lock|notifications|nightlight|top-bar|workspace-layout|battery-percentage|screensaver)|hardware($|\\.(laptop-display|mirror-display|touchpad($|\\.)|touchpad-haptics($|\\.)))|tests\\.(network-speedtest|disk-speedtest))|style\\.(theme|background|font|bar($|\\.position\\.top)|hyprland|screensaver($|\\.)|about($|\\.))|setup\\.(monitors|keybindings|input|network($|\\.)|default($|\\.)|plugin($|\\.)|security($|\\.)|config($|\\.)|direct-boot|reset)|install($|\\.)|remove($|\\.)|update($|\\.(omarchy|config($|\\.)|themes|process\\.shell|hardware($|\\.)|firmware|password($|\\.)|timezone|time)))")) then
