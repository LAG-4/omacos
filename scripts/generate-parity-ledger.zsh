...
jq --slurpfile keys "$keybindings_path" '
  ...
  def menu_parity:
    ...
    | if $item.referenceKind != "action" then
      ...
      elif ($item.id | test("^(system\\.hibernate|trigger\\.capture\\.screenrecord\\.webcam|trigger\\.hardware\\.(hybrid-gpu|touchscreen)|trigger\\.toggle\\.crash-capture|style\\.bar\\.position\\.(left|right)|style\\.unlock|update\\.process\\.hyprsunset|update\\.channel\\.(rc|dev))")) then
