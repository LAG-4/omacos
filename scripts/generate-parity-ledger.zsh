...
jq --slurpfile keys "$keybindings_path" '
  ...
  def menu_parity:
    ...
    | if $item.referenceKind != "action" then
      ...
      elif ($item.id | test("^(system\\.hibernate|trigger\\.capture\\.screenrecord\\.webcam|trigger\\.hardware\\.(hybrid-gpu|touchscreen)|trigger\\.toggle\\.(one-window-ratio|crash-capture)|style\\.bar\\.position\\.(left|right)|style\\.unlock|update\\.process\\.hyprsunset|update\\.channel\\.(rc|dev))")) then
      ...
      elif ($item.id | test("^(apps$|about$|system\\.|learn\\.|trigger\\.(emoji|reminder|capture\\.(screenshot|text|color|qr|screenrecord\\.(no-audio|desktop-audio|microphone|stop))|transcode|share($|\\.)|toggle\\.(idle-lock|notifications|nightlight|top-bar|window-gaps|workspace-layout|battery-percentage|screensaver)|hardware($|\\.(laptop-display|mirror-display|touchpad($|\\.)|touchpad-haptics($|\\.)))|tests\\.(network-speedtest|disk-speedtest))|style\\.(theme|background|font|bar($|\\.(position\\.(top|bottom)|transparency))|hyprland|screensaver($|\\.)|about($|\\.))|setup\\.(monitors|keybindings|input|network($|\\.)|default($|\\.)|plugin($|\\.)|security($|\\.)|config($|\\.)|direct-boot|reset)|install($|\\.)|remove($|\\.)|update($|\\.(omarchy|channel($|\\.(stable|edge))|config($|\\.)|themes|process\\.shell|hardware($|\\.)|firmware|password($|\\.)|timezone|time)))")) then
