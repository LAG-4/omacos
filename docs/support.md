...
| Overlapping OMacOS window groups | Accessibility substitute | Accessibility substitute | Accessibility substitute | Accessibility substitute |
| Focused-window opacity | Unavailable | Unavailable | Upstream error | Supported |
...
The shell is installed as an app bundle so macOS can present clear Microphone, Camera, and Speech Recognition prompts for dictation and webcam capture. It is signed with Hardened Runtime and narrow audio-input and camera entitlements. Accessibility and Input Monitoring cover native window actions and the shared Right Option pointer layer. Development installs use an ad-hoc identity. Tagged production archives use the automated Developer ID and notarization flow documented in [releasing.md](releasing.md); without one stable production identity, macOS may treat an updated executable as a new privacy client and ask again.
